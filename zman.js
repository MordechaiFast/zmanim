...
/** Returns the time of day for a fractional Julian Day,
 *  adjusted for standard time zones */
function JDtoCivilTime(JulianDay, location) {
  let time = (JulianDay - .5) % 1;
  time *= 24; // Convert to hours
  time += location.timezone + location.dst;
  if (time > 24)
    time -= 24
  else if (time < 0)
    time += 24;
...
function twilightTime(elevation, evening, date, location) {
  ...
  return JDtoCivilTime(current, location);
}
...
function hourAngleTwillight(h, evening) {
  const date = {year, month, day};
  const location = {lat, long, timezone, dst};
  sun_time = twilightTime(90-h, evening == 12, date, location)
  return HoursMinutesSeconds(sun_time);
}

function getAccurate(hour) {
  const date = {year, month, day};
  const location = {lat, long, timezone, dst};
  const atmospheric = {pressure, temp};
  let time = (
